...
---
글을 작성하고 난 이후에 position 계산 로직을 CPU -> GPU로 옮겼는데 CPU 사용률이 10 ~ 20% 로 더 최적화되었다. 100만개의 Particle도 프레임 드랍 없이 동작했으나, CPU에서 particle을 생성하는 탓에 조금의 딜레이는 발생한다. 아마도 생성 로직까지 GPU를 활용하면 메모리, CPU 사용량을 더 줄일 수 있지 않을까??
